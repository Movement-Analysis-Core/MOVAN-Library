function mvnx = loadMVNX20210204(filename)
% mvnx = loadMVNX(filename)
% inputs  - filename, file path to an mvnx file.
% outputs - mvnx, structure containing data from the mvnx file.
% Remarks
% - This code was originally received from XSENS to load their mvnx-files
%   into MATLAB.
% Future Work
% - None.
% Feb 2021 - Edited by Ben Senderling, bmchmovan@unomaha.edu
%          - Edited comments and changed function name from
%            load_mvnx_files.
% May 2015 - Created by XSENS

%% check filename
if ~contains(filename,'mvnx')
        ...
        try
            ...
            if any(strcmp(startwords,word{iWord}))
                ...
                if iscell(wordvalue{iWord}) || ~isempty(wordvalue{iWord}) && all(~isnan(wordvalue{iWord}))
                    setvalue(index(iWord),wordvalue{iWord},name(1:index(iWord)),wordfields{iWord},namecounter);
                end
...
function setvalue(index,value,name,fields,namecounter)
